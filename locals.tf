locals {
  ...
  #RT
  public_route_table_ids = [for k,v in lookup(lookup(module.subnets, "public", null), "aws_route_table", null) : v.id]
  app_route_table_ids = [for k,v in lookup(lookup(module.subnets, "app", null), "aws_route_table", null) : v.id]
  db_route_table_ids = [for k,v in lookup(lookup(module.subnets, "db", null), "aws_route_table", null) : v.id]
